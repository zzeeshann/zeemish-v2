...
- **Four audit dimensions:**
  ...
  3. **Essence not reference** (binary — THE PRIMARY BAR). No proper nouns from the piece, no dates, no quoted phrases, no industry-label tells, no "according to the piece". Checked against the piece's body excerpt for overlap.
...
- **Output:** `InteractiveGeneratorResult` — `{pieceId, date, skipped, declined, committed, auditorMaxFailed, interactiveId, slug, title, concept, questionCount, revisionCount, roundsUsed, voiceScore, finalAudit, tokensIn, tokensOut, durationMs}`. Surfaced back to Director for metered logging via `observer.logInteractiveGeneratorMetered`. **Four terminal states:**
  - `skipped`: `daily_pieces.interactive_id` already set (idempotent re-run, no Claude call)
  - `declined`: Claude returned the empty shape in any round — "this concept is too redundant with recent interactives"
  - `committed`: a round passed full audit; file + D1 writes landed with `voice_score` + `revision_count` populated from the final audit
  - `auditorMaxFailed`: 3 rounds exhausted without passing; **no commit, no D1 row**. Retry via admin button / `/interactive-generate-trigger`
- **Abandon on max-fail (not ship-as-low):** interactives are structural (unlike prose, no "mostly-fine salvage" from a max-failed round); permanence rule stays clean; retry is trivial (`interactive_id IS NULL` → Generator runs fresh on next trigger); no quality-flag-low state to filter from readers or the 4.6 last-beat prompt. See DECISIONS 2026-04-24 "Area 4 sub-task 4.5 — InteractiveAuditorAgent + abandon-on-max-fail".
- **Write path:** On a passing round, commits `content/interactives/<slug>.json` via Publisher's `publishToPath` (refuses overwrite, same mechanic as daily-piece ship). Then INSERTs an `interactives` row (content_json NULL — file is source of truth per 4.2; voice_score + revision_count populated from the passing round) and UPDATEs `daily_pieces.interactive_id`. Slug collision resolution: if the base slug exists, tries `-2`, `-3`, … up to `-5`; throws if all taken.
...
- **Idempotent:** short-circuits with `skipped: true` if `daily_pieces.interactive_id` is already set. Decline path returns without commit or D1 write. Max-fail path returns without commit or D1 write.
