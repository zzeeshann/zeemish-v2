...
Zeemish is not the model. Zeemish is a **system** built around the model. The system decides what text to send. The system remembers what the model said before. The system combines multiple calls into a workflow. The system stores the outputs in databases and publishes them to websites. The system has thirteen roles, quality gates, and a learning loop.
