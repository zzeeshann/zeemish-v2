...
- Drafter-declared `beatCount` in frontmatter can drift from actual `##` heading count in the MDX body (e.g. 2026-04-17 declares 6 but has 8 `##` headings). Reader UI counts actual headings in `src/lib/rehype-beats.ts` and is correct; the stored metadata in `daily_pieces.beat_count` is the Drafter's declared number. Consider adding a Structure-Editor gate: `beatCount` must equal number of `##` in body, or drop the frontmatter field and derive it at render time.
